...
contract ElasticToken is IERC20, Ownable {
    /// @dev token metadata
    string public name;
    ...
    function mintShares(address _user, uint256 _share) external onlyOwner {
        ...
        emit Transfer(address(0), _user, vault.amountForShare(address(this), _share));
    ...
    function burnShares(address _user, uint256 _share) external {
        ...
        emit Transfer(_user, address(0), vault.amountForShare(address(this), _share));
    ...
    function transferFrom(address _sender, address _recipient, uint256 _amount) external override(IERC20) returns (bool) {
    ...
    function _transfer(address _sender, address _recipient, uint256 _amount) internal {
        uint256 _sharesToTransfer = vault.sharesForAmount(address(this), _amount);
    ...
    function totalSupply() public view returns (uint256) {
        return vault.getTotalPooledEther(address(this));
    }

    function balanceOf(address _user) public view override(IERC20) returns (uint256) {
        return vault.getTotalEtherClaimOf(address(this), _user);
    }
}
